...
![CubeCL 并行层次由外到内：CubeCount、Cube（含 CubeDim）、Plane、Unit](../img/ch03-cube-hierarchy.svg)

与 CUDA 的粗略对照如下（由外到内）。`CubeDim` 是 Cube 的形状参数，
不是与 Cube、Plane 并列的一层：
...
![GPU 存储层次：host 内存/外部存储经总线到 device 全局内存，批量加载连续 tile 进入 cube 共享内存，最终到 unit 私有寄存器；越靠下容量越小、延迟越低](../img/ch03-memory-hierarchy.svg)
