...
本节的机制可以在没有任何框架的情况下复现。配套实验
`examples/ch02-mini-autodiff` 用约一百行 Rust 实现了一个标量版反向
tape：`Vec` 就是 tape、追加顺序就是拓扑序、反向按编号倒序扫描一遍、
`Mul` 的反向规则展示了 tape 为什么必须保存前向中间值。先在
[实验小节](07-labs.md)把它跑起来、亲手加一个算子，再回来读下面的
Burn 源码，会发现两边的结构一一对应——差别只在 Burn 要同时处理
张量、设备与并发。

## 源码导读

单个算子的反向规则长什么样、checkpoint 策略在哪里声明，
[「算子解剖」](../op-anatomy.md)以 tanh 为样本给出了逐层摘录与可
运行断言，适合与本节对照。想亲自核对本节描述，建议按以下顺序阅读本书固定版本的
