...
代入具体数字走一遍。设某层校准范围为 $[-3.2,\ 2.8]$，目标是
int8 非对称 $[-128, 127]$：

$$
s = \frac{2.8 - (-3.2)}{127 - (-128)} = \frac{6}{255} \approx 0.02353,
\qquad
z = \operatorname{round}\left(-128 - \frac{-3.2}{0.02353}\right)
= \operatorname{round}(8.0) = 8.
$$

真实值 $r = 0.5$ 被量化为
$q = \operatorname{round}(0.5 / 0.02353 + 8) = \operatorname{round}(29.25) = 29$；
反量化回 $(29 - 8) \times 0.02353 \approx 0.494$，绝对误差约 $0.006$。
这个误差不是意外，而是设计的直接后果：该层能用 int8 表达的最小间隔就是
$s \approx 0.0235$，任何落在中点的值都会损失最多 $s/2$。存储侧的收益同样
可以直接算：权重从 `f32` 到 int8 是 $4\times$ 压缩——7B 参数模型从约
$28\ \text{GB}$ 降到约 $7\ \text{GB}$，int4 再到约 $3.5\ \text{GB}$；但上一节
说过，只有 runtime 真的按 int8/int4 计算时，在线内存和延迟才会跟着降。
...
百分位校准把主体量化步长缩小约 22 倍（分辨率 $\propto 1/\mathrm{scale}$），
主体 MSE 大约按平方降到五百分之一。代价是被
...
开发中的 PTQ 路径支持有限的低精度表示。这不能写成“任意
