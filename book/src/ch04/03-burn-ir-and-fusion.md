...
逐操作执行通常要把 `t0` 写到全局内存，再由第二个 Kernel 读回。元素级
融合可以在同一 Kernel 中把中间值保留在局部值/寄存器，减少：

- 一次中间写入和读取；
- Kernel launch；
- 中间 allocation 和生命周期管理。
...
Fusion 包装。本章实验必须选择后者；把 Flex 输出正确当作 Fusion 已执行
是错误证据。
