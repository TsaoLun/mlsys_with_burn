...
本书首个候选版固定在 `pins.toml` 记录的 OpenMLSys 源码快照（source snapshot）
...
因此只作为源码证据和可选比较路径，不进入根 workspace 的 Cargo 依赖。
九章默认采用 CPU 可运行路径（CPU-first）；真实 GPU、NCCL、ONNX fixture、DDP、DQN/MARL
