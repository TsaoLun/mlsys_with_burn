...
1. train/validation 的排序后样本 ID 分别严格等于 `0` 到 `15` 和 `16` 到 `19`
   （代码里是半开区间 `0..16` 与 `16..20`），因而没有重复、重叠或遗漏；
...
例如把 batch size 从 4 改为 2 或 8 时，样本数仍应是 16/4，但 batch 数
会变。不要改成 5：验证集只有 4 个样本，末批会被 `FixBatchStrategy`
切成不完整形状，断言会直接失败，你看到的是报错而不是「变化」。如果你
只改断言而不解释为什么会变，这条实验还没有形成系统结论。你不需要提交
代码，只需要能用输出证明你理解了每个边界。
