...
真实网络排队，而是把跨机柜 pair 数量映射为一个确定性的 penalty。
这足以比较放置策略的协议趋势，不足以作为 GPU 集群 benchmark。

一个拓扑感知的放置器通常按以下顺序尝试：

1. 找到满足显存需求的空闲 GPU；
2. 优先在同一节点或同一机柜内凑齐成组资源；
3. 如果无法满足，再选择跨域组合，并估计新增的跨域通信；
4. 将选择结果和通信域写入作业记录，便于之后解释慢 step。
