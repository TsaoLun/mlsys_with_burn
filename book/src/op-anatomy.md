...
机制是否真的搞懂，有一个朴素的检验标准：随便挑一个算子，你能不能
说出**一次调用会牵动哪些文件、每一层为什么必须存在、缺了哪一层会
发生什么**。本页用 `tanh` 做一次完整解剖——它足够简单（一元、无
形状变化），又足够完整（有反向规则、有 GPU kernel、参与融合）。
所有摘录都来自本书固定版本源码，路径可逐行核对；页末的示例把关键
结论变成可运行断言。

## 一次调用牵动的十层

| # | 层 | 固定源码位置（相对 `burn/crates/`） | tanh 在这一层的形态 |
|---|---|---|---|
...
本页刻意不做：不修改上游源码，不在默认路径运行 GPU 层（第 6 层的
可观察版本见第 3 章的 wgpu 可选实验与第 4 章 FusionInspector）。
