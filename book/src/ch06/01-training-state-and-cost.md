...
加速转成全局等待。算一笔账：4 个设备各完成本步需 100 ms，只要其中
一个慢了 20%，整步就变成 120 ms——其余三个设备的 20 ms 全部变成
等待，集群利用率立刻从 100% 掉到约 $100/120 \approx 83\%$，而与
“快设备有多快”无关。模型更大时，通信 tensor 的字节数、设备间拓扑和
