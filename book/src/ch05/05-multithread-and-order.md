...
当 `num_workers > 0` 时，Burn 构造 `MultiThreadDataLoader`。从源码可以
追踪出如下路径：

![多 worker 数据通路：Dataset 经索引切分交给多个 worker 线程，产出经容量 100 的 mpsc 通道汇入主迭代器](../img/ch05-worker-pipeline.svg)

worker pool 在第一次迭代时懒初始化，并在同一个 loader 的后续 epoch 中
复用。消息通道容量是 100 个消息槽位，提供有限的背压；这是当前实现的
参数，不是跨版本的 API 承诺。
...
是否覆盖相同 item，并没有把 worker 消息到达顺序作为契约。因而准确的
结论是：
...
乱序可接受”，但不能直接迁移成 Burn 能力：Burn 的
`MultiThreadDataLoader` 使用消息接收顺序，没有对应的 Connector 序号
等待算法。因此本章把“保序性”作为系统设计问题和实验观察量来讲。
...
`items_processed` 自动代替 optimizer step 进度。Burn 的 DataLoader
负责迭代和错误传播；上述跨阶段提交协议要由训练系统自己设计。
