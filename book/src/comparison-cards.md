...
- **可运行观察**：`ch06-training-loop` 测试 weighted average、staleness、
  quorum、pipeline slots 和 checkpoint version；CPU autodiff loop 验证
  单设备训练。
...
- **可运行观察**：`ch07-record-roundtrip` 验证 Burnpack round-trip；
  同 crate 的纯 Rust contract harness 验证 checksum、版本、rollback 和
  dynamic batch。
