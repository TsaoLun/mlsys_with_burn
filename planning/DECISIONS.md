...
## D025：正文以系统课与实现落点为主，项目自洽后置

- 日期：2026-08-14
- 决策：读者主路径按 OpenMLSys 的系统问题组织九章，并补上当代训练并行、
  推理服务与集群控制面的成本模型。Burn / CubeCL / CubeK 用来指出
  「这一层行为写在哪个 crate」。第 8 章标为可选。项目自洽（版本钉扎、
  证据标签、对照矩阵、发布门禁）只留在附录、`planning/` 与工具链；
  章首、学习目标和系统结论不再以「可核对 / CPU-first / 快照」为叙事。
  新增书内页 `infra-map.md`（产业名字与章节对应）与 `crate-map.md`
  （一次调用经过的层）。默认示例仍在 CPU 上可运行，但不把这一点写成
  教材主题。
- 原因：前一阶段把可复核性做成了读者第一印象，冲淡了系统课本身，也
  让「打开框架源码」变成审计动作。需要把主路径还给问题、成本和实现
  落点，同时保留附录里的版本与来源账本。
